...
> **补充（2026-08-06）：** 统一安全等级、OS 沙箱全套建议、扩展签名、嵌入式 JS 能力与 Agent Workflow 限制，见同目录 [`unified-security-architecture.md`](./unified-security-architecture.md)。PerlicaScript JIT / deopt / 类型混淆契约见 [zixiao-labs/perlicascript `docs/jit-security.md`](https://github.com/zixiao-labs/perlicascript/blob/docs/jit-security/docs/jit-security.md)（待合入 `main`）。本文仍是四大特性的前置调研快照，不被全文替代。
>
