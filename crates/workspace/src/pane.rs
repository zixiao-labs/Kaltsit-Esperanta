    ...
    pub(super) fn file_names_for_prompt(
        ...
        for item in items {
            item.for_each_project_item(cx, &mut |_, project_item| {
                ...
                file_names.insert(filename.unwrap_or_else(|| tr!("Untitled").to_string()));
            });
        }
        if file_names.len() > 6 {
            format!(
        ...
        cx.spawn_in(window, async move |pane, cx| {
            ...
            if save_intent == SaveIntent::Close && dirty_items.len() > 1 {
                let answer = pane.update_in(cx, |_, window, cx| {
                    ...
                    window.prompt(
                        PromptLevel::Warning,
                        tr!("Do you want to save changes to the following files?").as_ref(),
                        Some(&detail),
                        &[
        ...
        if has_conflict && can_save {
            if has_deleted_file && is_singleton {
                let answer = pane.update_in(cx, |pane, window, cx| {
                    ...
                    window.prompt(
                                            PromptLevel::Warning,
                                            tr!("This file has been deleted on disk since you started editing it. Do you want to recreate it?").as_ref(),
                                            None,
                                            &[PromptButton::new(tr!("Save")), PromptButton::new(tr!("Close")), PromptButton::new(tr!("Cancel"))],
                        cx,
                    )
                })?;
                match answer.await {
                    ...
                    let answer_task = pane.update_in(cx, |pane, window, cx| {
                        if pane.save_modals_spawned.insert(item_id) {
                            ...
                            Some(window.prompt(
                                ...
                                None,
                                &[
                                    PromptButton::new(tr!("Save")),
                                    PromptButton::new(tr!("Don't Save")),
                                    PromptButton::new(tr!("Cancel")),
                                ],
                                cx,
                            ))
    ...
    fn render_tab(
        ...
        let tab = Tab::new(ix)
            ...
            .map(|this| {
                ...
                let end_slot_tooltip_text: SharedString;
                let end_slot = if is_pinned {
                    end_slot_action = &TogglePinTab;
                    end_slot_tooltip_text = tr!("Unpin Tab");
                    ...
                    end_slot_tooltip_text = tr!("Close Tab");
                ...
                .map(|this| {
                    if is_active {
                        ...
                        let tooltip_text = end_slot_tooltip_text.clone();
                        ...
                        this.tooltip(Tooltip::text(end_slot_tooltip_text.clone()))
                    }
                });
                    ...
                    if let Some(pane) = pane.upgrade() {
                        menu = menu
                            ...
                            .item(ContextMenuItem::Entry(
                                ContextMenuEntry::new(tr!("Close Others"))
                            ...
                            .extend(has_multibuffer_items.then(|| {
                                ContextMenuItem::Entry(
                                    ContextMenuEntry::new(tr!("Close Multibuffers"))
                            ...
                            .separator()
                            .item(ContextMenuItem::Entry(
                                ContextMenuEntry::new(tr!("Close Left"))
                            ...
                            .item(ContextMenuItem::Entry(
                                ContextMenuEntry::new(tr!("Close Right"))
                            ...
                            .separator()
                            .item(ContextMenuItem::Entry(
                                ContextMenuEntry::new(tr!("Close Clean"))
            ...
            PopoverMenu::new("pane-tab-bar-popover-menu")
                ...
                .menu(move |window, cx| {
                    Some(ContextMenu::build(window, cx, |menu, _, _| {
                        menu.action(tr!("New File"), NewFile.boxed_clone())
                            .action(tr!("Open File"), ToggleFileFinder::default().boxed_clone())
                            .separator()
                            .action(tr!("Search Project"), DeploySearch::default().boxed_clone())
                            .action(tr!("Search Symbols"), ToggleProjectSymbols.boxed_clone())
                            .separator()
                            .action(tr!("New Terminal"), NewTerminal::default().boxed_clone())
                            .action(
            ...
            PopoverMenu::new("pane-tab-bar-split")
                ...
                .menu(move |window, cx| {
                    ContextMenu::build(window, cx, |menu, _, _| {
                        ...
                        if can_split_move {
                            menu.action(tr!("Split Right"), SplitRight { mode }.boxed_clone())
                                .action(tr!("Split Left"), SplitLeft { mode }.boxed_clone())
                                .action(tr!("Split Up"), SplitUp { mode }.boxed_clone())
                                .action(tr!("Split Down"), SplitDown { mode }.boxed_clone())
                        } else {
                            menu.action(tr!("Split Right"), SplitRight::default().boxed_clone())
                                .action(tr!("Split Left"), SplitLeft::default().boxed_clone())
                                .action(tr!("Split Up"), SplitUp::default().boxed_clone())
                                .action(tr!("Split Down"), SplitDown::default().boxed_clone())
                        }
                    })
                    .into()
                }),
