...
fn files_not_created_on_launch(errors: HashMap<io::ErrorKind, Vec<&Path>>) {
    let message = "ZetaCode failed to launch";
...
fn fail_to_open_window(e: anyhow::Error, _cx: &mut App) {
    eprintln!(
        "ZetaCode failed to open a window: {e:?}. See https://zed.dev/docs/linux for troubleshooting steps."
    );
    ...
    #[cfg(any(target_os = "linux", target_os = "freebsd"))]
    {
        ...
        _cx.spawn(async move |_cx| {
            ...
            proxy
                .add_notification(
                    notification_id,
                    Notification::new("ZetaCode failed to launch")
                        .body(Some(
                            format!(
...
fn main() {
    ...
    if args.system_specs {
        ...
        println!("ZetaCode System Specs (from CLI):\n{}", system_specs);
        return;
    }
