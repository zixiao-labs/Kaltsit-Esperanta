...
async fn install_script(cx: &AsyncApp) -> Result<Option<PathBuf>> {
    let cli_path = cx.update(|cx| cx.path_for_auxiliary_executable("zeta"))?;
    let link_path = Path::new("/usr/local/bin/zeta");
...
pub fn install_cli_binary(window: &mut Window, cx: &mut Context<Workspace>) {
    const LINUX_PROMPT_DETAIL: &str = "If you installed ZetaCode from our official release add ~/.local/bin to your PATH.\n\nIf you installed from a different source like your package manager, then you may need to create an alias/symlink manually.";
    ...
    cx.spawn_in(window, async move |workspace, cx| {
        ...
        let path = match install_script(cx.deref()).await {
            ...
            Ok(None) => return Ok(()),
            Err(error) => {
                log::error!("failed to install zeta CLI: {error:#}");
                        ...
                        |cx| {
                            cx.new(|cx| {
                                MessageNotification::new(
                                    "You can add `zeta` to your PATH manually.",
                                    cx,
                                )
                                .with_title("Couldn't install the Zeta CLI")
        ...
        workspace.update_in(cx, |workspace, _, cx| {
            struct InstalledZetaCli;

            workspace.show_toast(
                Toast::new(
                    NotificationId::unique::<InstalledZetaCli>(),
                    format!(
                        "Installed `zeta` to {}. You can launch {} from your terminal.",
    ...
    .detach_and_prompt_err("Cannot install the Zeta CLI", window, cx, |_, _, _| None);
}
